  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      ...
      <View className="flex-1 justify-center items-center px-6">
        ...
        {/* Sign Up — Primary (solid) */}
        <Pressable onPress={() => router.push("/signup")} className="w-full max-w-xs">
          {({ pressed }) => (
            <View className={`px-8 py-4 rounded-2xl shadow-md ${pressed ? "bg-purple-700 shadow-purple-400/50" : "bg-purple-600 shadow-purple-300/50"}`}>
        ...
        <Pressable onPress={() => router.push("/login")} className="w-full max-w-xs mt-6">
          {({ pressed }) => (
            <View className={`px-8 py-4 rounded-2xl border-2 ${pressed ? "border-purple-700 bg-purple-50" : "border-purple-600 bg-white"}`}>
              <Text className={`text-center text-base font-semibold ${pressed ? "text-purple-700" : "text-purple-600"}`}>
                Login
              </Text>
            </View>
          )}
